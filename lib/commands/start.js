...
const BrowserTarget        = require('../targets/browser/target');
...
const supportedPlatforms   = ['ios', 'android', 'browser'];
...
module.exports = Command.extend({
  ...
  selectDevice(platforms, opts = {}) {
    return this.getInstalledDevices(platforms).then((devices) => {
      ...
      if (opts.platform === 'browser' && platforms.includes('browser')) {
        return new Device({
          name: 'Browser',
          deviceType: 'browser',
          platform: 'browser'
      ...
      if (!platforms.includes('browser') && devices.length === 0) {
        /* eslint-disable max-len */
